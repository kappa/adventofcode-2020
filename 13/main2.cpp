...
constexpr
long modulo(const long a, const long b) {
  const auto result = a % b;
  return result >= 0 ? result : result + b;
}

constexpr
long power_mod(const long base, const int power, const int mod) {
...
constexpr
long mulinv_mod(const long num, const int mod) {
...
int main()
{
  ...
  const long m =
    std::accumulate(buses.begin(), buses.end(), 1l, [](const long acc, const bus_t& bus) {
      return acc * bus.a;
    });

  const long x = std::accumulate(buses.begin(), buses.end(), 0l, [m](const long acc, const bus_t &bus) {
